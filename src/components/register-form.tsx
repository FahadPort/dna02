...
export default function RegisterForm() {
	...
	const onSubmits = async (data: TregisterFormData) => {
		await axios
			.post(
				`https://demo-clienttesting.com/mobiledna/backend/api/register`,
				data,
			)
			.then((response) => {
							...
							<div className="w-full flex items-center justify-between gap-5">
								<div className="w-full flex flex-col gap-2">
									<input
										...
										{...register("name")}
										placeholder="Name"
										className={`w-full bg-[#3A364D] text-white placeholder:text-[#6D6980] rounded-lg p-4 focus:border-[#F4404A] focus:border-[1px] focus:outline-none focus:ring-1 outline-none ${
