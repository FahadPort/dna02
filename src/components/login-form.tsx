...
import { ancestryTesting } from "../assets/index.ts";
...
export default function LoginForm() {
	...
	const onSubmits = async (data: TloginFormData) => {
		await axios
			.post(`https://demo-clienttesting.com/mobiledna/backend/api/login`, data)
			.then((response) => {
					...
					<div className="w-full px-10 flex justify-center flex-col gap-8">
						<div className="flex flex-col gap-4">
							...
							<div className="flex items-center gap-2">
								<button className="text-sm text-[#F4404A] font-normal leading-tight tracking-tight">
						...
						<form
							...
							<div className="flex flex-col gap-5">
								<div className="flex flex-col gap-2">
									<input
										...
										{...register("email")}
										placeholder="Email"
										className={`bg-[#3A364D] text-white placeholder:text-[#6D6980] rounded-lg p-4 focus:border-[#F4404A] focus:border-[1px] focus:outline-none focus:ring-1 outline-none ${
								...
								<div className="flex flex-col gap-2">
									...
									<Link
										className="text-[#F4404A] text-sm font-normal leading-tight tracking-tight pt-2 hover:underline"
										to="/reset">
										Forgot password
									</Link>
								</div>
							...
							<input
								...
								className="w-full rounded-lg p-4 text-[16px] text-white font-normal text-center leading-tight tracking-tight cursor-pointer"
